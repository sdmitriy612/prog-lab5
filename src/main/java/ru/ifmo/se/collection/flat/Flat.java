...
public class Flat implements Comparable<Flat>{
    ...
    public Flat(
        ...
        if(house.getName() == null) house = null;
        this.house = house;
    }
    ...
    public Flat(String name, Coordinates coordinates, Float area, int numberOfRooms, Furnish furnish,
        ...
        long lastFlatID = 0;
        String filePath = "LastFlatID.txt";

        try(FileIOController fileIOController = new FileIOController(filePath, false)){

            String fileContent = fileIOController.safeRead();

            if(fileContent != null && !fileContent.isBlank()) lastFlatID = Long.parseLong(fileContent);

            fileIOController.safeWrite(String.valueOf(lastFlatID + 1));

        }catch (Exception e){
            throw new RuntimeException(e.getMessage());
        }

        this.id = lastFlatID + 1;
    ...
    public int compareTo(Flat flat) {
        return Double.compare(this.area / this.numberOfRooms, flat.getArea() / flat.getNumberOfRooms());
    }
